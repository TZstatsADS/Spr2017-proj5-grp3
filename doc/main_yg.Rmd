...
ED.indina=apply(ED.continuous,c(1,2), function(x) 
  if(any(x %in% c("-9","-8","-7","-6","-5","-3","-2","-1",NA))) {x}else{0} )
...
ED.continuous[is.na(ED.continuous)]=0
#replace NA in continuous data with median (actually can replace with any number)

anyNA(ED.continuous)

#combine the data
ED.final<-cbind(challengeID=ED.continuous$challengeID,ED.categorical,ED.continuous[,-1],ED.indina)
