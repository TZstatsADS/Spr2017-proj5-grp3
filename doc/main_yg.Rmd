...
categorical=c(categorical,colnames(ED.indina))
...
continuous=c(colnames(ED.cont2),colnames(ED.factor[,1:7]))
...
write.csv(ED.final,file="~/GitHub/Spr2017-proj5-grp3/data/NAreplaced.csv",row.names = FALSE)

save(categorical,continuous, file="featuretype.RData")
...
data.train[,categorical]=as.data.frame(lapply(data.train[,categorical],factor))
...
colnames(Bdf) = c("Codes", "Description", "Decision","Score")
write.csv(Bdf, "../data/features.csv")
```
