...
ED=ED[,which(unlist(lapply(ED, function(x)!(sum(is.na(x))/obs>=0.8)))),with=F]
ED=ED[,which(unlist(lapply(ED, function(x)!(sum(x %in% c("-9","-6","-3"))/obs>=0.8)))),with=F]
...
extract.data <- extract.data[ apply(extract.data, 1, function(x){!sum(is.na(x))/cols>=0.7}),]
