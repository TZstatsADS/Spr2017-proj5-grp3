...
factor1<-function(x){
  return(factor(x,levels=as.integer(unique(x))))
}
```
...
for(i in cat.idx){
  data.train[,cat.idx]<-lapply(data.train[,cat.idx], factor)
  data.filtered[,cat.idx]<-lapply(data.filtered[,cat.idx], factor)
}
...
# SVM
svm_fit<-svm(grit~., data=data.train)
svm_predictions<-predict(svm_fit,newdata=data.filtered)
# Random Forest
rf <- randomForest(grit~., data=data.train)
