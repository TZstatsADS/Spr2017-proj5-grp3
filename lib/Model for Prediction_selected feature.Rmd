...
select <- read.csv('codes_description_importance.csv')
...
data.filtered <- subset(data.filtered,!is.na(data.filtered$gpa))# 2330*65

# create training and test data set
set.seed(123)
train.index <- sample(1:nrow(data.filtered),2000,replace = F)
train <- data.filtered[train.index,] #2000*65
test <- data.filtered[-train.index,] #330*65
```
...
```
Conclusion:
All the models have the same error.???
